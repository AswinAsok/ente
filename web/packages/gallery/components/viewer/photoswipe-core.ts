...
> {
    ...
    constructor({
        ...
        pswp.on("uiRegister", () => {
            ...
            const zoomIndex = ui.uiElementsData.findIndex(
                (e) => e.name == "zoom",
            );
            ui.uiElementsData[zoomIndex] = {
                ...ui.uiElementsData[zoomIndex]!,
                order: 6,
            };
            ...
            const renderDownloadProgress = () => {
                ...
                if (size.hidden != !sizeText) size.hidden = !sizeText;
                if (separator.hidden != !(sizeText || failed))
                    separator.hidden = !(sizeText || failed);
                if (retry.hidden != !failed) retry.hidden = !failed;
                const width = `${Math.max(state?.pct ?? 0, 1)}%`;
                if (fill.style.width != width) fill.style.width = width;
                const offset = `${100 - (state?.pct ?? 0)}`;
                if (progressRing.style.strokeDashoffset != offset)
